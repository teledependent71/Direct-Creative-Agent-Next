...
import postsPageInitialPaths52fcaResource from '../../resources/posts-page-initial-paths-52fca'
import postsPageInitialPropsCf451Resource from '../../resources/posts-page-initial-props-cf451'
  ...
  return (
    <>
      <div className="posts11-container">
        <Head>
          <title>Posts1 - Direct Creative Agent</title>
          <meta property="og:title" content="Posts1 - Direct Creative Agent" />
        </Head>
        <DataProvider
...
export async function getStaticPaths() {
  try {
    const response = await postsPageInitialPaths52fcaResource({})
    return {
...
export async function getStaticProps(context) {
  try {
    const response = await postsPageInitialPropsCf451Resource({
      ...context?.params,
    })
    if (!response?.data?.[0]) {
      return {
