...
import authorsPageInitialPaths85b9aResource from '../../resources/authors-page-initial-paths-85b9a'
import authorsPageInitialPropsC7616Resource from '../../resources/authors-page-initial-props-c7616'
  ...
  return (
    <>
      <div className="authors11-container">
        <Head>
          <title>Authors1 - Direct Creative Agent</title>
          <meta
            property="og:title"
            content="Authors1 - Direct Creative Agent"
          />
        </Head>
        <DataProvider
...
export async function getStaticPaths() {
  try {
    const response = await authorsPageInitialPaths85b9aResource({})
    return {
...
export async function getStaticProps(context) {
  try {
    const response = await authorsPageInitialPropsC7616Resource({
      ...context?.params,
    })
    if (!response?.data?.[0]) {
      return {
